...
static void PITCallback(void* args)
{
  ...
  {
    // Try to update the channel, if unsuccessful dont send analog packets
    if (Analog_Get(i))
    {
      TAnalogInput input = Analog_Input[i];

      // From the spec:
      // When SYNCHRONOUS: Send every 10ms
      // When ASYNCHRONOUS: Send when value has changed, at intervals no greater than 10ms
      if (TowerProtocolMode == SYNCHRONOUS || (input.oldValue.l != input.value.l))
      {
        SendAnalogValue(i, input.value);
      }
    }
  }
}
...
/*! @brief Initialises the Packet, Flash, LED, RTC, PIT, FTM and Analog modules.
 *
