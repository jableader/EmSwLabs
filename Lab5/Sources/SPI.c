...
bool SPI_Init(const TSPIModule* const aSPIModule, const uint32_t moduleClock)
{
  ...
  SPI2_MCR &= ~SPI_MCR_ROOE_MASK;    // Receive overflow overwrite, incoming data is ignored
...
{
  // The lower two bits represent what want our chip select to be
...
void SPI_ExchangeChar(const uint16_t dataTx, uint16_t* const dataRx)
{
  ...
  // Extract the received data from the input buffer
  uint16_t recievedByte = SPI2_POPR; // Use stack so we read from POPR regardless of whether we need to return it (as ROOE is cleared)
  if (dataRx != NULL)
    *dataRx = recievedByte;
